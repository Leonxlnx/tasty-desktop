...
- Reads trusted user MCP definitions from the selected Windows Kimi instance's effective `KIMI_CODE_HOME`
- Discovers project MCP only from `.mcp.json` and `.kimi-code/mcp.json` beneath the canonical project root
- Requires explicit local approval of a fingerprint covering the canonical root, fixed file presence, and exact bytes; any change requires reapproval
- Rejects project MCP policy changes while affected runtimes have active or pending work and resets those runtimes before applying the new policy
- Does not support project MCP approval for WSL Kimi instances
- Preserves trusted user definitions unless an approved, attachable project definition overrides the same name; unsupported project entries cannot suppress them, and the built-in preview name remains app-owned
- Keeps raw MCP URLs, headers, environment values, arguments, and credentials out of the renderer and remote clients

Optional remote access uses a separate listener that is disabled by default. Pairing codes are single-use and expire after ten minutes. Device tokens are random, stored only as hashes, rate-limited, and individually revocable. Remote devices use a restricted chat-control method set and cannot invoke login, updates, terminal, Git, filesystem, skill installation, diagnostics export, remote administration, or project MCP approval and revocation. Project MCP approval metadata is also omitted from remote capability responses. Kimi Code Desktop does not open firewall or router ports and does not provide a public relay.

Background-task records must resolve beneath the matching Kimi session and contain a validated finite task identifier. Monitoring expires after 24 hours and excludes tools that explicitly disable their timeout. A terminal state and bounded `outputPath` are recorded and surfaced to the UI after recovery. Completion never enqueues a prompt or invokes a model. The user must explicitly inspect the output or ask Kimi to inspect it.
