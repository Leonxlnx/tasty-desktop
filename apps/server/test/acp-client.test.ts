...
import { AcpClient, isTransientWindowsSpawnError, sanitizeWslChildEnvironment, type RuntimeEvent } from "../src/acp-client.js";
...
describe("AcpClient", () => {
  ...
  it("scrubs Windows home variables and their WSLENV forwarding entries", () => {
    const original = {
      PATH: "safe-path",
      SafeToken: "safe-token",
      kimi_code_home: "C:\\private\\kimi",
      HOME: "C:\\FixtureProfiles\\ExampleUser",
      Xdg_Config_Home: "C:\\private\\xdg",
      WslEnv: "PATH/p:KIMI_CODE_HOME/p:home:XDG_CONFIG_HOME:XDG_CUSTOM_HOME/u:SafeToken/u",
    };

    expect(sanitizeWslChildEnvironment(original)).toEqual({
      PATH: "safe-path",
      SafeToken: "safe-token",
      WslEnv: "PATH/p:SafeToken/u",
    });
    expect(original.kimi_code_home).toBe("C:\\private\\kimi");
    expect(sanitizeWslChildEnvironment({ WSLENV: "HOME:XDG_DATA_HOME/u" })).toEqual({});
  });
