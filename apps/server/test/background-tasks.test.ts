...
describe("background task monitoring", () => {
  ...
  it("rejects a sessions junction outside KIMI_CODE_HOME", async () => {
    const home = await mkdtemp(join(tmpdir(), "kimi-background-home-link-"));
    const outside = await mkdtemp(join(tmpdir(), "kimi-background-sessions-outside-"));
    await writeTask(
      join(outside, "wd-test", "session-own", "agents", "main", "tasks", "bash-build1.json"),
      { taskId: "bash-build1", description: "Outside", status: "completed", detached: true },
    );
    await symlink(outside, join(home, "sessions"), process.platform === "win32" ? "junction" : "dir");
  ...
  it("scans the top-level sessions root once per monitor pass", async () => {
    const home = await mkdtemp(join(tmpdir(), "kimi-background-index-"));
    await writeTask(
      join(home, "sessions", "wd-test", "session-own", "agents", "main", "tasks", "bash-build1.json"),
      { taskId: "bash-build1", description: "Build APK", status: "running", detached: true },
    );
    await writeTask(
      join(home, "sessions", "wd-test", "session-own", "agents", "main", "tasks", "bash-test1.json"),
      { taskId: "bash-test1", description: "Run tests", status: "running", detached: true },
    );
    const sessionsRoot = await realpath(join(home, "sessions"));
    const monitor = new BackgroundTaskMonitor({
      pending: () => [
        { threadId: "thread-1", sessionId: "session-own", taskId: "bash-build1", registeredAt: new Date(0).toISOString(), kimiHome: home },
        { threadId: "thread-2", sessionId: "session-own", taskId: "bash-test1", registeredAt: new Date(0).toISOString(), kimiHome: home },
      ],
    ...
    await monitor.checkNow();
    expect(readdirSpy.mock.calls.filter(([path]) => path === sessionsRoot)).toHaveLength(1);
  ...
  it("isolates identical task identities across Kimi homes", async () => {
    const firstHome = await mkdtemp(join(tmpdir(), "kimi-background-home-a-"));
    const secondHome = await mkdtemp(join(tmpdir(), "kimi-background-home-b-"));
    for (const home of [firstHome, secondHome]) {
      await writeTask(
        join(home, "sessions", "wd-test", "session-own", "agents", "main", "tasks", "bash-build1.json"),
        { taskId: "bash-build1", description: "Build APK", status: "completed", detached: true, exitCode: 0 },
      );
    }
    const pending: PendingBackgroundTask[] = [
      { threadId: "thread-a", sessionId: "session-own", taskId: "bash-build1", registeredAt: new Date(0).toISOString(), kimiHome: firstHome },
      { threadId: "thread-b", sessionId: "session-own", taskId: "bash-build1", registeredAt: new Date(0).toISOString(), kimiHome: secondHome },
    ];
    ...
    await monitor.checkNow();
    expect(finished.sort()).toEqual(["thread-a", "thread-b"]);
  });
  ...
  it("waits for an in-flight monitor pass when closing", async () => {
    vi.useFakeTimers();
    let release!: () => void;
    const gate = new Promise<void>((resolve) => { release = resolve; });
    const monitor = new BackgroundTaskMonitor({
