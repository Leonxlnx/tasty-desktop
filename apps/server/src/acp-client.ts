...
const MAX_SESSION_LIST_RESULTS = 10_000;
...
export type AcpClientOptions = {
  ...
  scrubWslHomeEnvironment?: boolean;
...
export function sanitizeWslChildEnvironment(environment: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const sanitized = { ...environment };
  const isWindowsHomeVariable = (name: string) => {
    const normalized = name.toUpperCase();
    return normalized === "KIMI_CODE_HOME" || normalized === "HOME" || normalized.startsWith("XDG_");
  };
  for (const key of Object.keys(sanitized)) {
    if (isWindowsHomeVariable(key)) delete sanitized[key];
  }
  for (const key of Object.keys(sanitized)) {
    if (key.toUpperCase() !== "WSLENV") continue;
    const entries = sanitized[key]?.split(":").filter((entry) => {
      const name = entry.split("/", 1)[0];
      return name !== undefined && name.length > 0 && !isWindowsHomeVariable(name);
    }) ?? [];
    if (entries.length) sanitized[key] = entries.join(":");
...
export class AcpClient {
  ...
  async start(): Promise<acp.InitializeResponse> {
    if (this.#connection || this.#closing || (this.#child && !hasExited(this.#child))) {
    ...
    const child = spawn(this.#options.binary, this.#options.args ?? ["acp"], {
      env: this.#options.scrubWslHomeEnvironment ? sanitizeWslChildEnvironment(environment) : environment,
    ...
    return this.#controlRequest("initialize", () => connection.initialize({
      ...
      clientInfo: { name: "kimi-code-desktop", title: "Kimi Code", version: "0.12.0" },
    }));
  }
  ...
  async newSession(cwd: string): Promise<acp.NewSessionResponse> {
    ...
    const result: acp.NewSessionResponse = await this.#controlRequest(
      "session/new",
  ...
  async listSessions(cwd?: string): Promise<acp.ListSessionsResponse> {
    ...
    let pages = 0;
    while (true) {
      if (pages >= MAX_SESSION_LIST_PAGES) {
        throw new Error(`ACP session/list exceeded ${MAX_SESSION_LIST_PAGES} pages`);
      }
      pages += 1;
      const page: acp.ListSessionsResponse = await this.#controlRequest(
        "session/list",
        () => this.#agent().listSessions({ ...(agentRoot ? { cwd: agentRoot } : {}), ...(cursor ? { cursor } : {}) }),
      );
      if (page.sessions.length > MAX_SESSION_LIST_RESULTS - sessions.length) {
        throw new Error(`ACP session/list exceeded ${MAX_SESSION_LIST_RESULTS} sessions`);
      }
      sessions.push(...page.sessions);
      cursor = page.nextCursor ?? undefined;
      if (!cursor) break;
      if (seenCursors.has(cursor)) throw new Error("ACP session/list returned a repeated cursor");
      seenCursors.add(cursor);
    }
    const result: acp.ListSessionsResponse = { sessions };
  ...
  async cancel(sessionId: string): Promise<void> {
    ...
    if (process.env.KIMI_FAKE === "1") {
      const delay = Number(process.env.KIMI_FAKE_CANCEL_DELIVERY_DELAY_MS ?? 0);
      if (Number.isFinite(delay) && delay > 0) {
        await new Promise((resolvePromise) => setTimeout(resolvePromise, Math.min(delay, 30_000)));
      }
      if (process.env.KIMI_FAKE_CANCEL_REJECT === "1") throw new Error("Fake cancel delivery failed");
    }
  ...
  async #terminateOwnedChild(child: ChildProcess | undefined): Promise<void> {
    try {
      await terminateChild(child);
      if (this.#child === child) this.#child = undefined;
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      void Promise.resolve().then(() => this.#options.onEvent({
        type: "diagnostic",
        level: "error",
        message: `Could not terminate ACP process tree: ${message}`,
      })).catch(() => undefined);
      throw error;
    }
  }
...
async function terminateChild(child: ChildProcess | undefined): Promise<void> {
  ...
  if (process.platform === "win32" && child.pid) {
    ...
    let killerError: Error | undefined;
    killer.once("error", (error) => { killerError = error; });
    const killerExited = await waitForExit(killer, CHILD_EXIT_GRACE_MS);
    if (!killerExited) {
      killer.kill("SIGKILL");
      await waitForExit(killer, CHILD_EXIT_GRACE_MS);
    }
    if (await waitForExit(child, CHILD_EXIT_GRACE_MS)) return;
    if (killerError) throw new Error(`taskkill could not start: ${killerError.message}`);
    if (!killerExited) throw new Error("taskkill did not exit before the shutdown deadline");
    if (killer.exitCode === 1 && !processExists(pid)) return;
    if (killer.exitCode !== 0) throw new Error(`taskkill exited with code ${killer.exitCode ?? "unknown"}`);
    throw new Error("ACP child process remained alive after taskkill succeeded");
  } else {
    child.kill("SIGTERM");
    if (!await waitForExit(child, CHILD_EXIT_GRACE_MS)) child.kill("SIGKILL");
  }
  if (!await waitForExit(child, CHILD_EXIT_GRACE_MS)) {
    throw new Error("ACP child process remained alive after forced termination");
  }
}
