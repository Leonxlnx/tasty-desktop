...
export async function readKimiBackgroundTask(kimiHome: string, sessionId: string, taskId: string): Promise<BackgroundTaskResult | { status: "running"; description: string } | undefined> {
  ...
  const index = await indexKimiSessions(kimiHome, [sessionId]);
  return readIndexedKimiBackgroundTask(index, sessionId, taskId);
}

async function indexKimiSessions(kimiHome: string, sessionIds: Iterable<string>): Promise<KimiSessionIndex | undefined> {
  const requested = new Set<string>();
  for (const sessionId of sessionIds) {
    if (!safeSegment(sessionId)) continue;
    for (const sessionDirectory of sessionDirectories(sessionId)) requested.add(sessionDirectory);
  }
  if (requested.size === 0) return undefined;
  let homeRoot: string;
  let sessionsRoot: string;
  try {
    homeRoot = await realpath(resolve(kimiHome));
    sessionsRoot = await realpath(join(homeRoot, "sessions"));
  } catch {
    return undefined;
  }
  if (!inside(homeRoot, sessionsRoot)) return undefined;
  const workspaces = await readdir(sessionsRoot, { withFileTypes: true }).catch(() => []);
  const sessionRoots = new Map<string, string[]>();
  for (const workspace of workspaces) {
    if (!workspace.isDirectory()) continue;
    for (const sessionDirectory of requested) {
      ...
      const roots = sessionRoots.get(sessionDirectory) ?? [];
      roots.push(sessionRoot);
      sessionRoots.set(sessionDirectory, roots);
    }
  }
  return { sessionsRoot, sessionRoots };
}

async function readIndexedKimiBackgroundTask(index: KimiSessionIndex | undefined, sessionId: string, taskId: string): Promise<BackgroundTaskResult | { status: "running"; description: string } | undefined> {
  if (!index || !safeSegment(sessionId) || !TASK_ID.test(taskId)) return undefined;
  const matches: Array<{ record: KimiTaskRecord; taskPath: string }> = [];
  for (const sessionDirectory of sessionDirectories(sessionId)) {
    for (const sessionRoot of index.sessionRoots.get(sessionDirectory) ?? []) {
      ...
      for (const agent of agents) {
        ...
        if (!taskPath || !validTaskPath(index.sessionsRoot, taskPath, sessionDirectory, taskId)) continue;
  ...
  if (!TERMINAL_STATUSES.has(String(record.status))) return record.status === "running" || record.status === "pending"
    ...
    : undefined;
  const outputPath = await safeOutputPath(index.sessionsRoot, taskPath, taskId);
...
function sessionDirectories(sessionId: string): string[] {
  return sessionId.startsWith("session_") ? [sessionId] : [sessionId, `session_${sessionId}`];
}
...
export class BackgroundTaskMonitor {
  ...
  constructor(options: {
    ...
    this.#runOperation = options.runOperation ?? ((operation) => operation());
  ...
  async checkNow(): Promise<void> {
    ...
    operation = this.#checkNow().finally(() => this.#operations.delete(operation));
  ...
  async #checkNow(): Promise<void> {
    ...
    const pendingByHome = new Map<string, PendingBackgroundTask[]>();
    for (const task of pending) {
      const key = JSON.stringify([task.kimiHome, task.threadId, task.sessionId, task.taskId]);
      if (this.#checking.has(key)) continue;
      const tasks = pendingByHome.get(task.kimiHome) ?? [];
      tasks.push(task);
      pendingByHome.set(task.kimiHome, tasks);
    }
    const indexes = new Map(await Promise.all([...pendingByHome].map(async ([home, tasks]) => [
      home,
      await indexKimiSessions(home, tasks.map((task) => task.sessionId)),
    ] as const)));
    for (const task of pending) {
      const key = JSON.stringify([task.kimiHome, task.threadId, task.sessionId, task.taskId]);
      if (this.#checking.has(key)) continue;
      ...
      try {
        ...
        const result = await readIndexedKimiBackgroundTask(indexes.get(task.kimiHome), task.sessionId, task.taskId);
  ...
  #schedule(delay: number): void {
    ...
    this.#timer = setTimeout(() => {
      this.#timer = undefined;
      this.#running = true;
      void this.#runOperation(() => this.checkNow())
        .catch(this.#onError)
        .finally(() => {
          this.#running = false;
          if (this.#closed) return;
