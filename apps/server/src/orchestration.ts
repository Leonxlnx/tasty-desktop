...
import { redactPrivateErrorText } from "./diagnostics.js";
...
export type RuntimeIdentity = { provider: ProviderId; instanceId?: string };
...
export type SubmissionReceipt = {
  ...
  fingerprint: string;
  queuedId: string;
  state: "queued" | "started" | "completed" | "removed" | "payload_lost";
  acceptedAt: string;
  updatedAt: string;
  turnId?: string;
};
...
export type BackgroundTask = {
  ...
  turnId: string;
  description: string;
  /** Internal provenance for local polling. Never expose this path outside the server. */
  kimiHome?: string;
...
export type DomainEvent =
  ...
  | { type: "ThreadCreated"; payload: { sessionId: string; creationId?: string; creationFingerprint?: string; provider?: ProviderId; instanceId?: string; parentThreadId?: string; cwd: string; worktree?: ThreadWorktree; kind?: "project" | "chat"; title: string; configOptions?: SessionConfigOption[] } }
  ...
  | { type: "TurnSubmissionAccepted"; payload: { submissionId: string; fingerprint: string; queuedId: string } }
  | { type: "TurnSubmissionsRemoved"; payload: { submissionIds: string[] } }
  | { type: "TurnSubmissionsPayloadLost"; payload: { submissionIds: string[] } }
  ...
  | { type: "BackgroundTaskRegistered"; payload: { taskId: string; queuedId: string; turnId: string; description: string; kimiHome?: string } }
  | { type: "BackgroundTaskFinished"; payload: { taskId: string; status: Exclude<BackgroundTask["status"], "running">; endedAt?: number; exitCode?: number | null; outputPath?: string } }
...
export class OrchestrationEngine {
  ...
  append(threadId: string, event: DomainEvent): Promise<StoredEvent> {
    const operation = this.#tail.then(async () => {
      const durableEvent = sanitizePersistedEvent(event);
      if (durableEvent.type === "ThreadCreated" || durableEvent.type === "ThreadSnapshot") {
        if (this.#threads.has(threadId)) throw new Error(`Thread ${threadId} already exists`);
        const payload = durableEvent.type === "ThreadCreated" ? durableEvent.payload : durableEvent.payload.thread;
        this.assertSessionAvailable(payload.sessionId, threadId, {
          provider: payload.provider ?? "kimi",
          ...(payload.instanceId ? { instanceId: payload.instanceId } : {}),
        });
      }
      const stored = await this.#store.append(threadId, durableEvent);
      this.#apply(stored);
      try {
        this.#publish(stored);
      } catch (error) {
      ...
      if (durableEvent.type === "TurnCompleted" || durableEvent.type === "TurnCancelled" || durableEvent.type === "ThreadDeleted") await this.#compactNow();
      return stored;
    });
  ...
  async #compactNow(): Promise<void> {
    ...
    for (const thread of compacted) {
      ...
      const key = runtimeSessionKey(thread.sessionId, thread);
      if (!this.#threadBySession.has(key)) this.#threadBySession.set(key, thread.threadId);
    }
  }
  ...
  assertSessionAvailable(sessionId: string, threadId: string, source: RuntimeIdentity): void {
    const thread = this.#threads.get(threadId);
    if (thread && (thread.sessionId !== sessionId || thread.provider !== source.provider || thread.instanceId !== source.instanceId)) {
      throw new Error(`Thread ${threadId} belongs to a different ACP runtime or session`);
    }
    const owner = this.#threadBySession.get(runtimeSessionKey(sessionId, source));
    if (owner && owner !== threadId) {
      throw new Error(`ACP session ${sessionId} is already owned by thread ${owner}`);
    }
  }

  runtimeThreadForSession(sessionId: string, source: RuntimeIdentity): Pick<ThreadProjection, "threadId" | "activeTurnId" | "provider" | "instanceId"> | undefined {
    const threadId = this.#threadBySession.get(runtimeSessionKey(sessionId, source));
  ...
  #apply(event: StoredEvent): void {
    if (event.type === "ThreadSnapshot") {
      ...
      const key = runtimeSessionKey(thread.sessionId, thread);
      if (!this.#threadBySession.has(key)) this.#threadBySession.set(key, event.threadId);
      return;
    }
    if (event.type === "ThreadCreated") {
      ...
      const thread = {
        ...
        ...(payload.creationId ? { creationId: payload.creationId } : {}),
        ...(payload.creationFingerprint ? { creationFingerprint: payload.creationFingerprint } : {}),
        provider: payload.provider ?? "kimi",
        ...(payload.instanceId ? { instanceId: payload.instanceId } : {}),
    ...
    if (event.type === "ThreadDeleted") {
      ...
      const key = runtimeSessionKey(thread.sessionId, thread);
      if (this.#threadBySession.get(key) === event.threadId) {
        this.#threadBySession.delete(key);
        const replacement = [...this.#threads.entries()].find(([, candidate]) => runtimeSessionKey(candidate.sessionId, candidate) === key);
        if (replacement) this.#threadBySession.set(key, replacement[0]);
      }
      return;
    }
    ...
    switch (event.type) {
      ...
      case "ThreadArchived":
        ...
        break;
      case "TurnSubmissionAccepted":
        if (!thread.submissionReceipts.some((receipt) => receipt.submissionId === payload.submissionId)) {
          thread.submissionReceipts.push({
          ...
          thread.submissionReceipts = pruneSubmissionReceipts(thread.submissionReceipts, event.createdAt, thread.activeTurnId);
        }
        break;
      case "TurnSubmissionsRemoved": {
        const removed = new Set(Array.isArray(payload.submissionIds) ? payload.submissionIds.map(String) : []);
        for (const receipt of thread.submissionReceipts) {
          if (!removed.has(receipt.submissionId) || receipt.state !== "queued") continue;
      ...
      case "TurnSubmissionsPayloadLost": {
        const lost = new Set(Array.isArray(payload.submissionIds) ? payload.submissionIds.map(String) : []);
        for (const receipt of thread.submissionReceipts) {
          if (!lost.has(receipt.submissionId) || receipt.state !== "queued") continue;
          receipt.state = "payload_lost";
          receipt.updatedAt = event.createdAt;
        }
        thread.submissionReceipts = pruneSubmissionReceipts(thread.submissionReceipts, event.createdAt, thread.activeTurnId);
        break;
      ...
      case "TurnStarted": {
        ...
        const receipt = typeof payload.sourceQueuedId === "string"
          ? thread.submissionReceipts.find((candidate) => candidate.queuedId === payload.sourceQueuedId)
          : undefined;
        if (receipt) {
          receipt.state = "started";
          receipt.turnId = String(payload.turnId);
          receipt.updatedAt = event.createdAt;
        }
      ...
      case "TurnCompleted":
        ...
        completeSubmissionReceipt(thread, String(payload.turnId), event.createdAt);
      ...
      case "BackgroundTaskRegistered":
        if (!thread.backgroundTasks.some((task) => task.taskId === payload.taskId)) {
          thread.backgroundTasks.push({
            ...
            ...(typeof payload.kimiHome === "string" ? { kimiHome: payload.kimiHome } : {}),
            status: "running",
      ...
      case "BackgroundTaskFinished": {
        ...
        if (task && task.status === "running") {
          ...
          if (typeof payload.outputPath === "string") task.outputPath = payload.outputPath;
        }
        break;
      }
  ...
  #reconcileSessionOwners(): void {
    ...
    for (const [threadId, thread] of this.#threads) {
      const key = runtimeSessionKey(thread.sessionId, thread);
      const owner = this.#threadBySession.get(key);
      if (!owner) {
        this.#threadBySession.set(key, threadId);
        continue;
      }
...
function runtimeSessionKey(sessionId: string, source: RuntimeIdentity): string {
  return JSON.stringify([source.provider, source.instanceId ?? null, sessionId]);
}
...
function sanitizePersistedEvent(event: DomainEvent): DomainEvent {
  if (event.type === "TurnPhaseChanged" && event.payload.error) {
    return { ...event, payload: { ...event.payload, error: redactPrivateErrorText(event.payload.error) } };
  }
  if (event.type === "TurnCompleted" && event.payload.error) {
    return { ...event, payload: { ...event.payload, error: redactPrivateErrorText(event.payload.error) } };
  }
  if (event.type === "BackgroundTaskReportCancelled" && event.payload.failure) {
    return { ...event, payload: { ...event.payload, failure: redactPrivateErrorText(event.payload.failure) } };
  }
  if (event.type === "ThreadSnapshot") {
    return { ...event, payload: { thread: sanitizeThreadErrors(event.payload.thread) } };
  }
  return event;
}

function sanitizeThreadErrors(thread: ThreadProjection): ThreadProjection {
  const sanitized = cloneThread(thread);
  if (sanitized.lifecycle?.error) sanitized.lifecycle.error = redactPrivateErrorText(sanitized.lifecycle.error);
  sanitized.turns = (sanitized.turns ?? []).map((turn) => turn.error
    ? { ...turn, error: redactPrivateErrorText(turn.error) }
    : turn);
  sanitized.backgroundTasks = (sanitized.backgroundTasks ?? []).map((task) => task.reportLastError
    ? { ...task, reportLastError: redactPrivateErrorText(task.reportLastError) }
    : task);
  return sanitized;
}
...
function compactThread(thread: ThreadProjection): ThreadProjection {
  const compacted = sanitizeThreadErrors(thread);
  ...
  compacted.submissionReceipts ??= [];
  compacted.submissionReceipts = pruneSubmissionReceipts(compacted.submissionReceipts, new Date().toISOString(), compacted.activeTurnId);
...
function pruneSubmissionReceipts(receipts: SubmissionReceipt[], now: string, activeTurnId?: string): SubmissionReceipt[] {
  const cutoff = Date.parse(now) - 30 * 24 * 60 * 60 * 1_000;
  const live = new Set(receipts.filter((receipt) => receipt.state === "queued" || receipt.state === "started" && receipt.turnId === activeTurnId));
  const retained = receipts.filter((receipt) => !live.has(receipt) && Date.parse(receipt.updatedAt) >= cutoff).slice(-256);
  const keep = new Set([...live, ...retained]);
  return receipts.filter((receipt) => keep.has(receipt));
}

function completeSubmissionReceipt(thread: ThreadProjection, turnId: string, completedAt: string): void {
  const receipt = thread.submissionReceipts.find((candidate) => candidate.state === "started" && candidate.turnId === turnId);
  if (receipt) {
    receipt.state = "completed";
    receipt.updatedAt = completedAt;
  }
  thread.submissionReceipts = pruneSubmissionReceipts(thread.submissionReceipts, completedAt, thread.activeTurnId);
}
