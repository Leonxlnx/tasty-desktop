...
export class RemoteAccess {
  ...
  async open(): Promise<void> {
    ...
    if (!loaded.value && loaded.corrupt) throw new Error("Remote access state is corrupt and no valid backup is available");
    if (!loaded.value) return;
    if (loaded.value.config.enabled && loaded.value.config.bind === "0.0.0.0") {
      const state = structuredClone(loaded.value);
      state.config = { ...state.config, enabled: false, bind: "127.0.0.1" };
      record(state, this.#now(), "configuration.migrated", undefined, "disabled insecure non-loopback binding");
  ...
  async configure(config: RemoteConfig): Promise<void> {
    ...
    if (parsed.enabled && parsed.bind === "0.0.0.0") throw new Error("Remote access requires TLS for non-loopback binding");
    await this.#mutate((state, pairings) => {
      state.config = parsed;
      pairings.clear();
      record(state, this.#now(), "configuration.updated", undefined, `${parsed.enabled ? "enabled" : "disabled"} ${parsed.bind}:${parsed.port}`);
    });
  }

  async createPairing(): Promise<{ code: string; expiresAt: string }> {
    return this.#mutate((state, pairings) => {
      for (const [code, expires] of pairings) if (expires <= this.#now()) pairings.delete(code);
      let code: string;
      do code = Array.from(randomBytes(8), (byte) => alphabet[byte! % alphabet.length]).join(""); while (pairings.has(code));
      const expires = this.#now() + 10 * 60_000;
      const expiresAt = new Date(expires).toISOString();
      pairings.set(code, expires);
      record(state, this.#now(), "pairing.created", undefined, `expires ${expiresAt}`);
  ...
  async claimPairing(code: string, name: string): Promise<{ device: RemoteDevice; token: string }> {
    return this.#mutate((state, pairings) => {
      ...
      const expires = pairings.get(normalized);
      ...
      const device: RemoteDeviceRecord = { id, name: cleanName, tokenHash: hash(token), createdAt, lastSeenAt: createdAt };
      pairings.delete(normalized);
      state.devices.push(device);
      record(state, this.#now(), "device.paired", id, cleanName);
  ...
  async seen(deviceId: string): Promise<void> {
    await this.#mutate((state) => {
      const device = state.devices.find((candidate) => candidate.id === deviceId && !candidate.revokedAt);
      if (!device) return;
      device.lastSeenAt = new Date(this.#now()).toISOString();
      record(state, this.#now(), "device.connected", deviceId, device.name);
    });
  }
  ...
  async revoke(deviceId: string): Promise<void> {
    await this.#mutate((state) => {
      ...
      record(state, this.#now(), "device.revoked", deviceId, device.name);
    });
  }
  ...
  async audit(action: string, deviceId?: string, detail?: string): Promise<void> {
    await this.#mutate((state) => record(state, this.#now(), action, deviceId, detail));
  }

  async #mutate<T>(change: (state: RemoteState, pairings: Map<string, number>) => T): Promise<T> {
    const operation = this.#mutation.then(async () => {
      const state = structuredClone(this.#state);
      const pairings = new Map(this.#pairings);
      const result = change(state, pairings);
      const next = stateSchema.parse(state);
      await writeRecoverableJson(this.#path, next);
      this.#state = next;
...
function record(state: RemoteState, now: number, action: string, deviceId?: string, detail?: string): void {
  state.audit.push({ id: randomUUID(), at: new Date(now).toISOString(), action: action.slice(0, 80), ...(deviceId ? { deviceId } : {}), ...(detail ? { detail: detail.slice(0, 200) } : {}) });
...
const remoteMethods = new Set([
  ...
  "runtime.configDefaults", "checkpoints.list", "usage.quota",
]);
