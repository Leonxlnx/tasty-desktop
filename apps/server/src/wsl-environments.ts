...
export class WslEnvironments {
  ...
  async list(): Promise<WslDistribution[]> {
    ...
    return Promise.all(names.map(async (name) => {
      ...
      const health = await run(this.binary, ["--distribution", name, "--exec", "sh", "-lc", "printf TASTY_WSL_OK"], 8_000);
      return { name, system, healthy: health.code === 0 && decode(health.stdout).includes("TASTY_WSL_OK"), ...(health.code === 0 ? {} : { message: bounded(decode(health.stderr) || "Health check failed") }) };
    }));
  }
