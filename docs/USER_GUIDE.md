...
Chat menu actions can rename, stop, archive, restore, open, or delete a chat. Archived chats appear in the reversible **Archived** section. The app refuses to archive a chat while turns, approvals, queued prompts, or background reports are pending. Archiving or deleting chat history never deletes a Git worktree or branch silently. Right-click opens the same menu as the row action.
...
Some Kimi shell and agent tools start finite tasks with automatic notification. The server monitors only an explicit task record from the same Kimi session for up to 24 hours. When that task finishes, the app queues a follow-up so Kimi can inspect the bounded output and report the verified result. Recovery is at least once, not exactly once.
...
Use Kimi's own commands, such as `/mcp-config` and `/update-config`, when the runtime advertises them. Plugin management appears only when Kimi exposes a matching command. Repository-controlled MCP files are review-only and never execute when a project opens. Sensitive MCP headers, environment values, arguments, and credentials stay in the server process.
...
The right work panel switches between:
...
The panel can be resized and moved through Layout settings. Its tabs share one compact header.

### Git changes

The Git manager supports status, diff, stage, unstage, commit, local branch creation and switching, upstream-aware push, fast-forward-only pull, HTTPS or SSH clone, and turn-level patch copying.
...
Finite tasks with automatic notification remain in the Agents projection after the original turn ends. When one reaches a terminal state, the app queues a follow-up for Kimi to inspect its same-session output. Arbitrary detached processes and development servers are not treated as completion notifications.
